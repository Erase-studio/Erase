...
class Sound {
  ...
  constructor() {
    ...
    if (this.on) {
      // Browsers only let audio start from a real gesture (a tap, a click, a
      // key — never a scroll), and iOS insists it happens inside that event.
      // Listen for every kind, and keep listening until one actually works.
      const events = ["pointerdown", "pointerup", "touchend", "click", "keydown"] as const;
      const wake = () => {
        if (!this.on) return;
        this.start();
        if (this.running) events.forEach((e) => window.removeEventListener(e, wake, true));
      };
  ...
  /** The gesture that's allowed to start audio has already happened. */
  wake() {
    if (this.on) this.start();
  }

  /** Actually making sound right now, not just switched on. */
  get running() {
  ...
  /** When this tap brought the sound up, so the same tap can't also put it down. */
  wokeAt = -1e9;

  private start() {
    if (!this.ctx || this.ctx.state !== "running") this.wokeAt = performance.now();
...
/**
 * Started by the facade in lib/sound.ts, which owns the on/off state and only
 * pulls this file in once someone has touched the page — audio can't start
 * before a gesture anyway, and a visitor who never makes one never pays for it.
 */
export { Sound };
